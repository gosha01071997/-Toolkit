...
import { checkEquipmentCompatibility, computeTableRows, equipmentMatchesType, getVisibleStages, isVisible, parseTablePaste, searchEquipment, tableToTsv, validateField } from "./engine.mjs";
  ...
  return <div className="guided-stack">
    {requirements.map(requirement => {
      ...
      return <section className="guided-panel" key={requirement.id}>
        ...
        <label>Выберите оборудование<select value={selected[requirement.id] || ""} onChange={event => onSelect(requirement.id, event.target.value)}><option value="">Не выбрано</option>{choices.map(choice => <option value={choice.id} key={choice.id}>{choice.name} · {choice.manufacturer || "производитель не указан"} · {choice.model || "модель не указана"}</option>)}</select></label>
        {item && <div className="equipment-summary"><b>{item.name}</b><dl><dt>Тип</dt><dd>{item.type || "Не указан"}</dd><dt>Производитель</dt><dd>{item.manufacturer || "Не указан"}</dd><dt>Модель</dt><dd>{item.model || "Не указана"}</dd><dt>Название экземпляра</dt><dd>{item.name}</dd><dt>Рабочий диапазон</dt><dd>{typeof item.frequencyRange === "string" ? item.frequencyRange : item.frequencyRange ? `${item.frequencyRange.min}–${item.frequencyRange.max}` : "Не указан"}</dd><dt>Калибровочные данные</dt><dd>{item.calibrationStatus || (item.calibrationCharacteristic ? "Есть данные" : "Нет данных")}</dd></dl>{!item.calibrationStatus && !item.calibrationCharacteristic && <p className="guided-warning">Калибровочные данные отсутствуют. Продолжить можно, но проверьте пригодность оборудования.</p>}{compatibility?.warnings.map(text => <p className="guided-warning" key={text}>{text}</p>)}</div>}
...
export default function GuidedTestRunner({ scenario, onOpenEquipment = () => {}, onBackToSelection }) {
  ...
  return <main className="guided-runner">
    ...
    <article className="guided-stage"><h2>{stage.heading || stage.title}</h2>
      {stage.id === "equipment" && <EquipmentStage requirements={scenario.equipmentRequirements} selected={progress.equipment} onAddEquipment={onOpenEquipment} onSelect={(id, value) => update({ equipment: { ...progress.equipment, [id]: value } })}/>}
      ...
      {stage.actions?.map(action => <section className="instruction" key={action.id}><h3>{action.title}</h3><p>{action.instruction}</p>{action.image && <img className="instruction-image" src={action.image} alt={action.imageAlt || action.title}/>} {action.explanation && <small>{action.explanation}</small>}{action.warning && <div className="guided-warning">{action.warning}</div>}<label className="check"><input type="checkbox" checked={Boolean(progress.completed[action.id])} onChange={e => update({ completed: { ...progress.completed, [action.id]: e.target.checked } })}/> {action.checkboxLabel || "Выполнено"}</label></section>)}
      {stage.derived?.map((item, index) => { const value = item.value(progress.inputs, progress); return value ? <div className={item.prominent ? "derived prominent" : "derived"} key={item.label || index}>{item.label && <span>{item.label}</span>}<strong>{value}</strong></div> : null; })}
      ...
      {result && <section className="guided-panel test-result"><h3>Результат испытания</h3><div className="category"><span>Категория</span><strong>{result.category}</strong></div><dl><dt>Минимальное расстояние D</dt><dd>{result.distance == null ? "—" : `${String(result.distance).replace(".", ",")} м`}</dd><dt>Контрольное отклонение Dc</dt><dd>{result.dc == null ? "—" : `${result.dc.toLocaleString("ru-RU", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}°`}</dd><dt>Окружающее поле H</dt><dd>{progress.inputs.h || "—"} А/м</dd><dt>Температура</dt><dd>{progress.inputs.temperature || "—"} °C</dd><dt>Влажность</dt><dd>{progress.inputs.humidity || "—"} %</dd><dt>Давление</dt><dd>{progress.inputs.pressure || "—"} кПа</dd><dt>Средство определения отклонения</dt><dd>{equipment.find(i => i.id === progress.equipment.deflectionInstrument)?.name || "Не выбрано"}</dd><dt>Магнитометр</dt><dd>{equipment.find(i => i.id === progress.equipment.magnetometer)?.name || "Не использовался"}</dd><dt>Испытуемое оборудование</dt><dd>{[progress.inputs.eutName, progress.inputs.eutModel].filter(Boolean).join(" · ") || "—"}</dd><dt>Режим ИО</dt><dd>{progress.inputs.maximumMode || progress.inputs.eutMode || "—"}</dd><dt>Метод</dt><dd>{progress.inputs.method === "sensor" ? "перемещался датчик" : progress.inputs.method === "eut" ? "перемещалось ИО" : "—"}</dd></dl>{!result.conditions.ok && <p className="guided-warning">Условия помещения выходили за стандартный диапазон.</p>}{result.uniformity && !result.uniformity.ok && <p className="guided-warning">Однородность магнитного поля вдоль траектории недостаточна.</p>}<p className="guided-muted">Прогресс испытания сохранён локально и восстановится при следующем открытии.</p></section>}
      ...
      <label className="stage-complete"><input type="checkbox" checked={Boolean(progress.completed[`stage:${stage.id}`])} onChange={e => update({ completed: { ...progress.completed, [`stage:${stage.id}`]: e.target.checked } })}/> Этап выполнен</label>
    </article>
