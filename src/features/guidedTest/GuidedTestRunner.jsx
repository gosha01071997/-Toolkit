...
export default function GuidedTestRunner({ scenario, onOpenEquipment = () => {}, onBackToSelection }) {
  ...
  return <main className="guided-runner">
    ...
    <nav className="stage-list" aria-label="Этапы испытания">{stages.map((item, index) => <button key={item.id} className={index === current ? "active" : ""} disabled={index > current + 1 || (index > current && !canAdvance)} onClick={() => update({ currentStage: index })}><span>{progress.completed[`stage:${item.id}`] ? "✓" : index + 1}</span>{item.title}</button>)}</nav>
    <article className="guided-stage"><h2>{stage.heading || stage.title}</h2>
      ...
      {result && <section className="guided-panel test-result"><h3>Испытание завершено</h3><div className="category"><span>Категория</span><strong>{result.category}</strong></div><div className="result-vitals"><b>Dc = {result.dc == null ? "—" : `${result.dc.toLocaleString("ru-RU", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}°`}</b>{result.category !== "Y" && <b>D = {String(result.distance).replace(".", ",")} м</b>}</div><dl><dt>Изделие</dt><dd>{progress.inputs.eutName || "—"}{progress.inputs.eutModel ? ` · ${progress.inputs.eutModel}` : ""}</dd><dt>Магнитный индикатор</dt><dd>{equipmentOptionLabels(equipment).get(progress.equipment.deflectionInstrument)}</dd>{progress.equipment.magnetometer && <><dt>Магнитометр</dt><dd>{equipmentOptionLabels(equipment).get(progress.equipment.magnetometer)}</dd></>}<dt>Магнитное поле H</dt><dd>{progress.inputs.h || "—"} А/м</dd><dt>Условия</dt><dd>{progress.inputs.temperature || "—"} °C · {progress.inputs.humidity || "—"} % · {progress.inputs.pressure || "—"} кПа</dd><dt>Худший режим</dt><dd>{progress.inputs.maximumMode || "Не зафиксирован"}</dd><dt>Способ перемещения</dt><dd>{progress.inputs.method === "sensor" ? "Магнитный индикатор" : "Изделие"}</dd><dt>Фотографии</dt><dd>{loadPhotos(localStorage, scenario.id).length}</dd>{progress.inputs.testNotes && <><dt>Заметки</dt><dd>{progress.inputs.testNotes}</dd></>}</dl>{!result.conditions.ok && <p className="guided-warning">Лабораторные условия выходили за стандартный диапазон.</p>}{result.uniformity && !result.uniformity.ok && <p className="guided-warning">Однородность магнитного поля вдоль траектории недостаточна.</p>}<p className="guided-muted">Категория фиксирует результат измерения и сама по себе не является заключением о соответствии требованиям.</p></section>}
