...
const MeasurementResult = ({ result }) => result && <section className="guided-panel measurement-result"><h3>Результат измерения</h3><div className="category"><span>Категория</span><strong>{result.category}</strong></div>{result.category === "Y" ? <p>{result.categoryRange}</p> : <dl><dt>Минимальное расстояние D</dt><dd>{String(result.distance).replace(".", ",")} м</dd><dt>Диапазон категории</dt><dd>{result.categoryRange}</dd></dl>}</section>;
