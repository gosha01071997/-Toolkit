...
import { checkEquipmentCompatibility, computeTableRows, equipmentMatchesType, getVisibleStages, parseTablePaste, searchEquipment, tableToTsv } from "./engine.mjs";
  ...
  return <div className="guided-stack">
    {requirements.map(requirement => {
      ...
      return <section className="guided-panel" key={requirement.id}>
        <h3>{requirement.title || requirement.type}</h3>
...
export default function GuidedTestRunner({ scenario, onOpenEquipment = () => {} }) {
  ...
  const field = (definition) => <label key={definition.id}>{definition.label}<input type={definition.type || "text"} placeholder={definition.placeholder} value={progress.inputs[definition.id] || ""} onChange={e => update({ inputs: { ...progress.inputs, [definition.id]: e.target.value } })}/>{definition.unit && <span className="field-unit">{definition.unit}</span>}</label>;
  ...
  return <main className="guided-runner">
    ...
    <article className="guided-stage"><h2>{stage.title}</h2>
      ...
      {stage.diagram && <section className="guided-panel diagram"><h3>{stage.diagram.title}</h3><button title="Нажмите, чтобы увеличить" onClick={e => e.currentTarget.classList.toggle("enlarged")}>{stage.diagram.image && <img src={stage.diagram.image} alt={stage.diagram.alt || "Схема испытания"}/>}<strong>{stage.diagram.description}</strong><span>{stage.diagram.labels?.join(" → ")}</span></button></section>}
      {stage.actions?.map(action => <section className="instruction" key={action.id}><h3>{action.title}</h3><p>{action.instruction}</p>{action.image && <img className="instruction-image" src={action.image} alt={action.imageAlt || action.title}/>} {action.explanation && <small>{action.explanation}</small>}{action.warning && <div className="guided-warning">{action.warning}</div>}<label className="check"><input type="checkbox" checked={Boolean(progress.completed[action.id])} onChange={e => update({ completed: { ...progress.completed, [action.id]: e.target.checked } })}/> Выполнено</label></section>)}
      ...
      {stage.id === "diagram" && <Photos scenarioId={scenario.id}/>} 
      {stage.id === "result" && <section className="guided-panel"><h3>Сводка</h3><p>Выполнено действий: {Object.values(progress.completed).filter(Boolean).length}</p><p>Выбрано единиц оборудования: {Object.values(progress.equipment).filter(Boolean).length}</p><p className="guided-muted">Незавершённое испытание сохранено локально и восстановится при следующем открытии.</p></section>}
