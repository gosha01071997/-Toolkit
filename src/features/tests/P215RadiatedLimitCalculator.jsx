...
function LimitChart({ mode }) {
  ...
  return <div style={{ overflowX: "auto" }}><svg role="img" aria-label={`График предельных линий ${mode === "BL" ? "B и L" : mode}`} viewBox="0 0 800 300" style={{ display: "block", minWidth: 620, width: "100%" }}>
    ...
    {series.map(item => <polyline key={item.category} points={item.points.map(point => `${x(point.frequencyMHz)},${y(point.limitDbUvM)}`).join(" ")} fill="none" stroke={item.category === "B" ? "#60A5FA" : "#34D399"} strokeWidth="3" />)}
    <text x="400" y="297" fill="#94A3B8" fontSize="12" textAnchor="middle">Частота, MHz (логарифмическая шкала)</text>
    {series.map((item, index) => <g key={`legend-${item.category}`}><line x1={620+index*65} x2={640+index*65} y1="22" y2="22" stroke={item.category === "B" ? "#60A5FA" : "#34D399"} strokeWidth="3"/><text x={645+index*65} y="26" fill="#F8FAFC" fontSize="12">{item.category}</text></g>)}
  </svg></div>;
}
...
export default function P215RadiatedLimitCalculator() {
  ...
  return <section style={panel}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
      <label>Категория<select aria-label="Категория" value={category} onChange={event => setCategory(event.target.value)} style={input}><option>B</option><option>L</option></select></label>
      ...
      <div><span>Результат</span><div aria-live="polite" style={{ fontSize: 21, fontWeight: 800, marginTop: 8, color: result.inRange ? "#34D399" : "#F59E0B" }}>{result.inRange ? formatLimit(result.limitDbUvM) : "Вне диапазона"}</div></div>
    </div>
