...
import "./components/TestCards.css";
    ...
    <PageContainer>
      ...
      {tab === "info" && (
        <div>
          ...
          {test.diagram && <div style={{...styles.card,background:"#f8fbff",padding:12}}>
            ...
            <TestDiagram type={test.diagram} />
            <div style={{fontSize:11,color:"#64748b",marginTop:12}}>Авторская инженерная схема EMC Toolkit показывает функциональные типы оборудования и не заменяет применимую методику.</div>
          </div>}
          <div style={styles.card}>
...
function TestsScreen() {
  ...
  return <PageContainer>
    ...
    <div className="premium-list test-card-list">{allTests.map(t=><div key={t.id} className={`premium-card premium-card-action test-card ${!t.custom&&["p204","p205","p214","p215"].includes(t.id)?"is-child":""}`} onClick={()=>setSelected(t)} style={{borderLeft:"3px solid "+(t.custom?C.cyan:t.parent?"#A78BFA":"#7C8CFF")}}><div className="premium-icon-box test-card__number">{t.short}</div><div className="test-card__content"><div className="test-card__title">{t.name}</div><div className="test-card__description">Простыми словами: {t.simpleDescription||t.desc}</div><div className="test-card__meta">{t.custom?(t.standard||"Пользовательская методика"):t.standard}</div>{t.alternateName&&<div className="test-card__alias">Дополнительное обозначение: {t.alternateName}</div>}</div><div className="test-card__actions"><Button variant="secondary" onClick={e=>{e.stopPropagation();openEdit(t)}}>Редактировать</Button>{t.custom&&<Button variant="danger" onClick={e=>{e.stopPropagation();if(window.confirm("Удалить пользовательское испытание?")){const n=customTests.filter(x=>x.id!==t.id);setCustomTests(n);localStorage.setItem("emc_custom_tests_v1",JSON.stringify(n))}}}>Удалить</Button>}</div></div>)}</div>
  </PageContainer>;
}
