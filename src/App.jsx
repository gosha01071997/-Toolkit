...
const [instanceDraft, setInstanceDraft] = useState({serialNumber:"",arm:"",calibrationValidUntil:"",length:"",connectorFrom:"",connectorTo:""});
const [selected, setSelected] = useState(null);
...
const addCatalogEquipment = () => {
  ...
  try{localStorage.setItem("emc_custom_equip_v1",JSON.stringify(updated));}catch(e){}
  setCatalogOpen(false); setCatalogSelection(null); setInstanceDraft({serialNumber:"",arm:"",calibrationValidUntil:"",length:"",connectorFrom:"",connectorTo:""}); setSelected(item.id);
};
    ...
    <>
      {!compact && (
        <SectionHero
          ...
          subtitle="Приборы и испытательное оборудование вашей лаборатории."
          stats={[
      ...
      <SectionHeader title="Парк оборудования" caption="Конкретные физические экземпляры, доступные для испытаний" count={`${filtered.length} найдено`} accent="#60A5FA" />
      ...
      {catalogOpen&&(()=>{
        const detailModel=EQUIPMENT_CATALOG.find(x=>x.id===catalogDetail);
        const selectedModel=EQUIPMENT_CATALOG.find(x=>x.id===catalogSelection);
        const closeCatalog=()=>{setCatalogOpen(false);setCatalogSelection(null);setCatalogDetail(null)};
        const startAdding=model=>{setCatalogDetail(null);setCatalogSelection(model.id);setInstanceDraft(v=>({...v,arm:armFilter!=="Все"?armFilter:""}))};
        const results=searchCatalog(EQUIPMENT_CATALOG,catalogSearch,catalogType);
        return <div className="equipment-catalog-backdrop" role="presentation"><div role="dialog" aria-modal="true" aria-label="Добавить оборудование" className="premium-card equipment-catalog-modal">
          <header className="equipment-catalog-header"><div><h2>{selectedModel?"Новый экземпляр":detailModel?"Карточка модели":"Добавить оборудование"}</h2>{!selectedModel&&!detailModel&&<div className="equipment-catalog-kicker">Встроенный офлайн-каталог EMC Toolkit</div>}</div><Button variant="secondary" onClick={closeCatalog}>Закрыть</Button></header>
          {!selectedModel&&!detailModel&&<div className="equipment-catalog-tools"><input autoFocus style={styles.input} aria-label="Поиск в каталоге" placeholder="Производитель, модель или тип оборудования" value={catalogSearch} onChange={e=>setCatalogSearch(e.target.value)}/><select style={styles.select} aria-label="Фильтр по типу" value={catalogType} onChange={e=>setCatalogType(e.target.value)}><option value="">Все типы</option>{EQUIPMENT_TYPE_OPTIONS.map(([id,label])=><option value={id} key={id}>{label}</option>)}</select></div>}
          {!selectedModel&&!detailModel&&<div className="equipment-catalog-results">{results.length?results.map(model=><article key={model.id} className="equipment-catalog-card"><div><h3>{model.model}</h3><p>{typeLabel(model.equipmentType)} · {model.manufacturer}</p>{visibleSpecifications(model).slice(0,4).length>0&&<dl className="equipment-catalog-specs">{visibleSpecifications(model).slice(0,4).map(([key,value])=><div key={key}><dt>{key}</dt><dd>{String(value)}</dd></div>)}</dl>}</div><div className="equipment-catalog-actions"><Button variant="secondary" onClick={()=>setCatalogDetail(model.id)}>Подробнее</Button><Button onClick={()=>startAdding(model)}>Добавить</Button></div></article>):<div className="equipment-catalog-empty">По вашему запросу моделей не найдено.</div>}</div>}
          {detailModel&&<div className="equipment-catalog-detail"><div className="equipment-instance-summary"><strong>{detailModel.manufacturer} {detailModel.model}</strong><span>{typeLabel(detailModel.equipmentType)}{detailModel.subtype?` · ${detailModel.subtype}`:""}</span></div>{visibleSpecifications(detailModel).length>0&&<section><h3>Основные технические характеристики</h3><dl className="equipment-catalog-specs">{visibleSpecifications(detailModel).map(([key,value])=><div key={key}><dt>{key}</dt><dd>{String(value)}</dd></div>)}</dl></section>}{detailModel.applications?.length>0&&<section><h3>Назначение / применение</h3><ul>{detailModel.applications.map(value=><li key={value}>{value}</li>)}</ul></section>}{detailModel.compatibleAccessories?.length>0&&<section><h3>Совместимая оснастка</h3><ul>{detailModel.compatibleAccessories.map(value=><li key={String(value.id||value)}>{String(value.title||value)}</li>)}</ul></section>}<section><h3>Документация и источник данных</h3>{detailModel.sources?.map(source=><a key={source.url} href={source.url} target="_blank" rel="noreferrer">{source.title}</a>)}</section><div className="equipment-catalog-actions" style={{marginTop:16}}><Button variant="secondary" onClick={()=>setCatalogDetail(null)}>Назад</Button><Button onClick={()=>startAdding(detailModel)}>Добавить в лабораторию</Button></div></div>}
          {selectedModel&&<div className="equipment-instance-form"><div className="equipment-instance-summary"><strong>{selectedModel.model}</strong><span>{typeLabel(selectedModel.equipmentType)} · {selectedModel.manufacturer}</span></div><Field label="Серийный / заводской № (необязательно)"><input style={styles.input} value={instanceDraft.serialNumber} onChange={e=>setInstanceDraft({...instanceDraft,serialNumber:e.target.value})}/></Field>{selectedModel.equipmentType==="cable"&&<><Field label="Фактическая длина экземпляра"><input style={styles.input} value={instanceDraft.length} onChange={e=>setInstanceDraft({...instanceDraft,length:e.target.value})} placeholder="например, 3 м"/></Field><Field label="Разъём со стороны 1"><input style={styles.input} value={instanceDraft.connectorFrom} onChange={e=>setInstanceDraft({...instanceDraft,connectorFrom:e.target.value})}/></Field><Field label="Разъём со стороны 2"><input style={styles.input} value={instanceDraft.connectorTo} onChange={e=>setInstanceDraft({...instanceDraft,connectorTo:e.target.value})}/></Field></>}<Field label="АРМ / станция"><select style={styles.select} value={instanceDraft.arm} onChange={e=>setInstanceDraft({...instanceDraft,arm:e.target.value})}><option value="">Не назначено</option>{armsList.map(a=><option key={a}>{a}</option>)}</select></Field><Field label="Поверка / калибровка действительна до (необязательно)"><input type="date" style={styles.input} value={instanceDraft.calibrationValidUntil} onChange={e=>setInstanceDraft({...instanceDraft,calibrationValidUntil:e.target.value})}/></Field><div className="equipment-catalog-actions"><Button variant="secondary" onClick={()=>setCatalogSelection(null)}>Назад</Button><Button onClick={addCatalogEquipment}>Добавить в лабораторию</Button></div></div>}
          {!selectedModel&&!detailModel&&<footer className="equipment-catalog-footer"><span>Не нашли свою модель?</span><Button variant="secondary" onClick={()=>{closeCatalog();createEquipment()}}>Добавить оборудование вручную</Button></footer>}
        </div></div>
      })()}
      {deleteConfirmModal}
